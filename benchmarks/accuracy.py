...
def generate_update_args_multilabel(
    num_batches: int, batch_size: int, device: torch.device, num_classes: int
) -> Dict[str, Any]:
    input = generate_random_input(
        InputType.MULTILABEL, device, num_batches, batch_size, num_classes=num_classes
    )
    target = generate_random_input(
        InputType.MULTILABEL, device, num_batches, batch_size, num_classes=num_classes
    )
...
def main(num_batches, batch_size, compute_interval, num_classes, use_torchmetrics):
    ...
    rank_0_print(f"Benchmark BinaryAccuracy...")
    if use_torchmetrics:
        metric = Accuracy().to(device)
    else:
        metric = BinaryAccuracy(device=device).to(device)
    ...
    rank_0_print(f"Benchmark MulticlassAccuracy...")
    if use_torchmetrics:
        metric = Accuracy().to(device)
    else:
    ...
    rank_0_print(f"Benchmark MultiLabelAccuracy...")
    if use_torchmetrics:
        metric = Accuracy(multiclass=True, subset_accuracy=True).to(device)
    else:
        metric = MultilabelAccuracy(device=device)
    update_kwargs = generate_update_args_multilabel(
        num_batches, batch_size, device, num_classes
    )
    get_benchmark_stats(
        metric, num_batches, compute_interval, update_kwargs, use_torchmetrics
