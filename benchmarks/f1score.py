...
from torcheval.metrics import BinaryF1Score, Metric, MulticlassF1Score
...
def main(num_batches, batch_size, compute_interval, num_classes, use_torchmetrics):
    ...
    rank_0_print(f"Benchmark BinaryF1Score...")
    if use_torchmetrics:
        metric = F1Score().to(device)
    else:
        metric = BinaryF1Score(device=device).to(device)
