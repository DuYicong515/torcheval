...
from torcheval.metrics import BinaryPrecision, Metric, MulticlassPrecision
...
def main(num_batches, batch_size, compute_interval, num_classes, use_torchmetrics):
    ...
    if use_torchmetrics:
        metric = Precision().to(device)
    else:
        metric = BinaryPrecision(device=device)
    ...
    rank_0_print(f"Benchmark MulticlassPrecision...")
    if use_torchmetrics:
        metric = Precision().to(device)
    else:
        metric = MulticlassPrecision(device=device)
...
if __name__ == "__main__":
    print("Benchmark precision metrics...")
