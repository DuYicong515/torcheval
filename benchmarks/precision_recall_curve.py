...
def main(num_batches, batch_size, compute_interval, num_classes, use_torchmetrics):
    ...
    # rank_0_print(f"Benchmark BinaryPrecisionRecallCurve...")
    # if use_torchmetrics:
    #     metric = PrecisionRecallCurve().to(device)
    # else:
    #     metric = BinaryPrecisionRecallCurve(device=device)
    # update_kwargs = generate_update_args_binary(num_batches, batch_size, device)
    # get_benchmark_stats(metric, num_batches, compute_interval, update_kwargs, use_torchmetrics)

    rank_0_print(f"Benchmark MulticlassPrecisionRecallCurve...")
    if use_torchmetrics:
        metric = PrecisionRecallCurve(num_classes=4).to(device)
    else:
        metric = MulticlassPrecisionRecallCurve(num_classes=4, device=device)
    update_kwargs = generate_update_args_multiclass(
        num_batches, batch_size=8, device=device, num_classes=num_classes
    )
    get_benchmark_stats(
...
if __name__ == "__main__":
    print("Benchmark precision recall curve metrics...")
